...
### 🔍 0. 事前確認（ブロッカー排除）
- [ ] ブランチが `feature/phase4-security-permissions` である
- [ ] ルートからテスト実行でGreenであることを確認
  - [ ] `npm run test`
- [ ] 作業ツリーがクリーン
  - [ ] `git status`
...
1) Red: テスト作成（新規）
- [ ] `dev/tests/active/2025-08-08_csrf_protect_Test.php` を作成
- [ ] 仕様テスト
  - [ ] 発行モード: フィールドHTMLが返る（name=`ldl_delete_nonce` を含む）
  - [ ] 検証モード（一致）: `check_admin_referer` をモックし true → true を返す
  - [ ] 検証モード（不一致）: false → false を返す
- [ ] 実行: `npm run test`（失敗を確認）

2) Green: 最小実装
- [ ] `localize-debug-log/localize-debug-log.php` に `ldl_csrf_protect($action='ldl_delete_log_action', $field='ldl_delete_nonce', $mode='field')` を追加
  - [ ] `$mode==='field'` → `wp_nonce_field($action, $field)` の戻り値を返す
  - [ ] `$mode==='verify'` → `check_admin_referer($action, $field)` のboolを返す
- [ ] 実行: `npm run test`（合格を確認）

3) Refactor
- [ ] 簡潔化・PHPDoc 追加・例外対策（関数未定義環境でのフォールバック）
...
1) Red: テスト作成（新規）
- [ ] `dev/tests/active/2025-08-08_validate_log_path_Test.php` を作成
- [ ] 仕様テスト
...
2) Green: 最小実装
- [ ] `localize-debug-log/localize-debug-log.php` に `ldl_validate_log_path($path)` を追加
  - [ ] `realpath($path)` が得られる場合は `real_logs_dir` で前方一致チェック
  - [ ] `realpath($path)` が得られない場合は `realpath(dirname($path))` を用いてディレクトリで検証
  - [ ] 妥当時は正規化パス（ファイル不存在時は結合した正規化パス）を返す／不正時は `false`
- [ ] 実行: `npm run test`（合格を確認）

3) Refactor
- [ ] 比較時のディレクトリ区切り・大文字小文字差異（Windows想定）に配慮
...
- [ ] `dev/tests/active/2025-08-08_log_deletion_security_Test.php` を作成
- [ ] 仕様テスト
...
2) Green: 実装変更
- [ ] `ldl_handle_delete_request()` を改修
  - [ ] `$_SERVER['REQUEST_METHOD']==='POST'` 以外は return
  - [ ] `ldl_csrf_protect(...,'verify')` で検証
  - [ ] `ldl_validate_log_path(ldl_get_log_path())` を用いてパス検証
  - [ ] 失敗時は `$ldl_last_delete_result = ['success'=>false,'message'=>('permission'|'nonce'|'validate')]`
- [ ] 実行: `npm run test`（合格を確認）

3) Refactor
- [ ] 条件分岐の早期 return 化と可読性の向上
...
- [ ] 正常: 実行後にファイルが存在し、サイズが0である
- [ ] 失敗: `LOCK_EX` 取得不可時にフォールバックが走る（WP_Mockで分岐を誘導）
- [ ] 失敗: パス妥当性が `false` の場合は実行せず `success=false`（message='validate'）
- [ ] 実行: `npm run test`（失敗を確認）

2) Green: 実装変更
- [ ] 事前に `ldl_validate_log_path` でパス妥当性チェック
- [ ] 第一選択: `file_put_contents($path, '', LOCK_EX)`
- [ ] フォールバック: `fopen('c+')` → `flock(LOCK_EX)` → `ftruncate(0)` → `fflush` → `flock(LOCK_UN)` → `fclose`
- [ ] 実行: `npm run test`（合格を確認）

3) Refactor
- [ ] 例外安全化と戻り値の一貫性（`['success'=>bool,'message'=>string|null]`）

---

### 🔗 5. 統合確認（UI/通知のリグレッション回避）
- [ ] 既存 UI テスト（`2025-08-07_*` の active テスト）で Green を確認
- [ ] 削除後の通知表示（success/error）が従来どおり出ることを確認
- [ ] 大容量ログ（1MB超）警告表示が維持されていることを確認

---

### 📚 6. ドキュメント整備
- [ ] `_doc/_TechSpec.md` のセキュリティ節に「パス検証（realpath）」「排他制御（LOCK_EX/fo flock）」「POST限定」の説明を追記
- [ ] `_doc/_Milestone.md` Phase 4 にチェック付与と完了報告を追記

---

### ✅ 7. 完了チェック
- [ ] 全テスト Green（`npm run test`）
- [ ] 追加したテストファイルは `dev/tests/active/` に配置・命名規約準拠
- [ ] 既存関数・フック・UI の破壊的変更なし
- [ ] 関数プレフィックス `ldl_` の統一維持
- [ ] ドキュメント更新の反映確認
